...
def read_frame(mask_path):
    return cv2.imread(mask_path)




def vos_tracking_video(video_state, interactive_state, mask_dropdown, save_dataset_mode):
    ...
    # ❌ Error Handling: Ensure mask is not empty
    if len(np.unique(template_mask)) == 1:
        template_mask[0][0] = 1
        operation_log = [("Please add at least one mask to track by clicking the image in step 2.","Error"), ("","")]
        return None, video_state, interactive_state, operation_log, operation_log

    # ✅ Run Tracking Algorithm
    masks, logits, painted_images = model.generator(images=following_frames, template_mask=template_mask)

    # ✅ Save Masks Immediately if "Dataset Mode" is Enabled
    if save_dataset_mode:
        dataset_masks_path = "./dataset/masks/"
        os.makedirs(dataset_masks_path, exist_ok=True)

        print("✅ Saving tracked masks for dataset...")
        mask_frames = []

        for idx, mask in enumerate(masks):
            mask_path = os.path.join(dataset_masks_path, f"mask_{idx:04d}.png")
            mask = (mask * 255).astype(np.uint8)
            cv2.imwrite(mask_path, mask)
            mask_rgb = cv2.cvtColor(mask, cv2.COLOR_GRAY2RGB)
            mask_frames.append(mask_rgb)

        print(f"✅ Saved {len(masks)} mask frames for dataset.")

        output_dir = "./result/track/"
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, video_state['video_name'])

        video_output = generate_video_from_frames(mask_frames, output_path, fps)

    # ✅ Update Video State with Tracked Masks
    if interactive_state["track_end_number"]: 
...
# inpaint 
def inpaint_video(video_state, resize_ratio_number, dilate_radius_number, raft_iter_number, 
                  subvideo_length_number, neighbor_length_number, ref_stride_number, 
                  mask_dropdown, video_completion_mode):  
    ...
    # Iterate through all masks and remove the masks that are not in mask_dropdown
    unique_masks = np.unique(inpaint_masks)
    num_masks = len(unique_masks) - 1
    for i in range(1, num_masks + 1):
        if i in inpaint_mask_numbers:
            continue
        inpaint_masks[inpaint_masks == i] = 0  # Remove unselected masks

    # ✅ Modify Completion Mode to Use Masks Instead of Full Frame Inpainting
    if video_completion_mode:
        print("🎭 Video Completion Mode Enabled (Filling Masked Objects Instead of Removing)")
        
        # Instead of making everything missing, we keep the mask as-is
        # This ensures that only the masked areas are reconstructed
        inpaint_masks[inpaint_masks > 0] = 1  # Convert all non-zero mask values to 1

    # Perform inpainting with user masks
    ...
    # Check if inpainted_frames is valid
    if inpainted_frames is None or len(inpainted_frames) == 0:
        print("❌ Error: No inpainted frames generated. Skipping video saving.")
        return None, operation_log, operation_log

    # ✅ Ensure output directory exists
    output_dir = "./result/inpaint/"
    os.makedirs(output_dir, exist_ok=True)  

    output_path = os.path.join(output_dir, video_state["video_name"])

    print(f"Saving inpainted video to: {output_path}")  

    video_output = generate_video_from_frames(inpainted_frames, output_path, fps)
...
def generate_video_from_frames(frames, output_path, fps):
    # Convert fps to a standard float
    fps = float(fps)  # Ensure fps is not a NumPy float64

    # Debug: Print fps type
    print(f"FPS type: {type(fps)}, value: {fps}")

    # Ensure fps is a valid integer if needed
    if fps.is_integer():
        fps = int(fps)

    # Check if frames is not empty or corrupted
    if frames is None or len(frames) == 0:
        print("❌ Error: No frames provided. Skipping video saving.")
        return
...
with gr.Blocks(theme=gr.themes.Monochrome(), css=css) as iface:
            ...
            with gr.Accordion('ProPainter Parameters (click to expand)', open=False):
                ...
                with gr.Row():
                    ...
                    ref_stride_number = gr.Slider(label='Stride of global reference frames.',
                                            ...
                                            value=10,)
                with gr.Row():
                    video_completion_mode = gr.Checkbox(label="Enable Video Completion Mode", value=False)
                with gr.Row():
                    save_dataset_mode = gr.Checkbox(label="Enable Dataset Saving Mode", value=False)
    with gr.Column():
        # input video
    ...
    tracking_video_predict_button.click(
        ...
        inputs=[video_state, interactive_state, mask_dropdown, save_dataset_mode],  # ✅ Added save_dataset_mode
